...
#define EAGLE_INIT(__VERBOSE__) \
    Eagle *e = new Eagle();     \
    e->SetVerbose(__VERBOSE__)

// EAGLE_EXIT : Cleans up all memory
#define EAGLE_EXIT delete e
...
#define ADD_TEST(__COLLECTION_NAME__, __TEST_NAME__) \
    e->AddTest(#__COLLECTION_NAME__, #__TEST_NAME__, &__TEST_NAME__)

// RUN_ALL_TESTS : Runs all tests across all collections
#define RUN_ALL_TESTS() e->RunAll()

// RUN_COLLECTION : Runs a given collection
#define RUN_COLLECTION(__COLLECTION_NAME__) e->RunCollection(#__COLLECTION_NAME__)
...
#define RUN_TEST(__COLLECTION_NAME__, __TEST_NAME__) e->RunTest(#__COLLECTION_NAME__, #__TEST_NAME__)
